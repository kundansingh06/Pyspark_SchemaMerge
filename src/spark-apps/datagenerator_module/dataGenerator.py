...
class DataGenerator:
    ...
    """Input- data_path: path where the partition will be created (string)
    ...
    def gen_data_simple_schema(self, data_path, partition_date, num_rows, file_format):
        ...
        for _ in range(num_rows):
            df_temp = self.spark.createDataFrame([[person.identifier(),person.first_name(),person.last_name(),person.occupation(),partition_date]], schema_df)
            try:
                df = df.union(df_temp)
            except:
                df = df_temp
